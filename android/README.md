...
| C++17 host    | Used to build the vendored `scripts/octarine-icon-tool`, **only if** the project sets `icon=`. |
...
`windowSplashScreenBackground` for Android 12+. Without `icon=` the task warn-skips and the APK
ships the green-robot defaults + a launcher-icon flash on 12+.
