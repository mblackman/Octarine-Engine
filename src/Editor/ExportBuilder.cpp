...
std::optional<std::filesystem::path> ExportBuilder::ResolveBuildScript(const std::filesystem::path& project_dir,
                                                                       ExportTarget target) {
...
std::vector<std::string> ExportBuilder::Validate(const std::filesystem::path& project_dir, ExportTarget target) {
  ...
  if (!ResolveBuildScript(project_dir, target)) {
    const std::string filename = std::string(ScriptStem(target)) + kScriptExt;
    errors.push_back("missing scripts/" + filename + " — run scripts/octarine-init-build to scaffold it");
  }
