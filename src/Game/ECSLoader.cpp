...
void ECSLoader::LoadEntity(sol::table entityTable,
  ...
  if (hasComponents != sol::nullopt) {
    ...
    if (projectileEmitter != sol::nullopt) {
      newEntity.AddComponent<ProjectileEmitterComponent>(
          ...
          static_cast<int>(entityTable["components"]["projectile_emitter"]
                                      ["repeat_frequency"]
                                          .get_or(1)) *
              1000,
          static_cast<int>(entityTable["components"]["projectile_emitter"]
          ...
          static_cast<int>(
              entityTable["components"]["projectile_emitter"]["hit_damage"]
                  .get_or(10)),
          entityTable["components"]["projectile_emitter"]["friendly"].get_or(
              false));
    }
    ...
    sol::optional<sol::table> onUpdateScript =
        entityTable["components"]["on_update_script"];
    if (onUpdateScript != sol::nullopt) {
      sol::function scriptFunction =
          entityTable["components"]["on_update_script"][0];
