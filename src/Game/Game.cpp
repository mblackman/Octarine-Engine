...
bool Game::RunBakeValidation(const std::string& assetPath)
{
    ...
    if (wrote && TTF_Init())
    {
        ...
        for (const auto& [id, entry] : assetManager.GetCatalog().Entries())
        {
            ...
            {
                Logger::Warn(
                    "Bake: TTF_OpenFont failed for " + entry.fullPath + " @ " + std::to_string(entry.fontSize) +
                    " — skipping atlas.");
                continue;
            }
...
void Game::Setup()
{
    ...
    if (gameConfig.HasLoadedConfig())
    {
        ...
        if (allowManifest)
        {
            const std::string pakPath = (std::filesystem::path(assetManager.GetBasePath()) / "asset_bundle.pak").
                string();
