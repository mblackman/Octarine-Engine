...
LevelLoader::LevelLoader() { Logger::Info("Level Loader created."); }

LevelLoader::~LevelLoader() { Logger::Info("Level Loader destroyed."); }
...
void LevelLoader::LoadLevel(sol::state& lua,
  ...
  auto filePath =
      "./assets/scripts/level" + std::to_string(levelNumber) + ".lua";
  ...
  while (std::getline(file, line)) {
    ...
    for (const auto& item : row) {
      ...
      tile.AddComponent<TransformComponent>(
          glm::vec2(tileWidth * columnNumber * tileMapScale,
                    tileHeight * rowNumber * tileMapScale),
          glm::vec2(tileMapScale, tileMapScale), 0.0);
      tile.AddComponent<SpriteComponent>(
          tileMapTextureId, tileWidth, tileHeight, 0, false,
          tileWidth * columnIndex, tileHeight * rowIndex);
      columnNumber++;
    }
