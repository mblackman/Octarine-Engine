  ...
  static ScaleComponent fromLua(const sol::object& data) {
    ...
    if (data.is<sol::table>()) {
      ...
      if (t["value"].valid()) {
        ...
        x = LuaComponentHelpers::SafeGetOptionalValue<float>(t, "x", 1.0f);
        y = LuaComponentHelpers::SafeGetOptionalValue<float>(t, "y", 1.0f);
      }
      ...
      Logger::Warn(
          "scale component expects { x = , y = }, { value = { x = , y = } } or a bare number; "
          "got something else. Defaulting to (1, 1).");
    }
