...
namespace octarine::process
{
            ...
            for (const auto& [k, v] : env)
            {
                ...
                auto it = std::find_if(entries.begin(), entries.end(),
                                       [&](const std::wstring& e) {
                                           ...
                                           return CompareStringOrdinal(e.c_str(), static_cast<int>(wk.size()),
                                                                      wk.c_str(), static_cast<int>(wk.size()),
                                                                      TRUE) == CSTR_EQUAL;
                                       });
                if (it != entries.end())
                {
    ...
    std::optional<Process> Process::Spawn(const SpawnOptions& opts)
    {
        ...
        pid_t child = -1;
        const int rc = posix_spawnp(&child, opts.argv[0].c_str(), &actions, &attr,
                                    argv_c.data(), envp_ptr);
