...
{
    fps_metrics_.reserve(kFpsBuffer);
}
...
void PerfOverlaySystem::Draw(Registry& registry, SDL_Renderer* sdlRenderer, const float deltaTime)
{
    ...
    if (fps_metric_index_ > kFpsBuffer - 1)
    {
        fps_metric_index_ = 0;
    }
    ...
    const float avgFps = get_average(fps_metrics_);
    const float fps95 = PerfUtils::GetPercentileAverage(fps_metrics_, 0.95f);
    const float fps99 = PerfUtils::GetPercentileAverage(fps_metrics_, 0.99f);
    ...
    const SDL_FRect bg{
        origin.x - kPadding, origin.y - kPadding, blockW + (2.0F * kPadding), blockH + (2.0F * kPadding)
    };
