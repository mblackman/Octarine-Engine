...
struct AudioSourceComponent {
  ...
  // Spatial audio: when `spatial` is true, SpatialAudioSystem mutates the track's gain +
  ...
  // Doppler shift: when `doppler` is true AND the entity has a RigidBodyComponent,
  ...
  // Culling resume offset: -1 = no pending resume. AudioCullingSystem stashes
